...
//Calculation for Salary after taxes
    let salaryAfterTaxes = grossPay * (1-taxRate) 

//Rounding down the salary after taxes 
    salaryAfterTaxes = (Math.floor(salaryAfterTaxes));

//Calculation for Salary after expenses
    let savings = salaryAfterTaxes - expenses
...
// Calculate remaining material after consumption
let remainingMaterial = totalMaterial - totalConsumed;

// Return the remaining material formatted with units (no space between number and unit)
...
function interest(principal, rate, periods) {
    ...
    //Calculating the income earned from 
    let amountGainedFromInvestment= principal * (rate * periods);

    let finalValueOfInvestment = principal + amountGainedFromInvestment

    finalValueOfInvestment = (Math.floor(finalValueOfInvestment));

    return finalValueOfInvestment
